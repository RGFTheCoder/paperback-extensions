...
export class ComixExtension implements ExtensionImpl<typeof ComixConfig> {
  ...
  private async updateFilters(force: boolean): Promise<void> {
    ...
    if (cached && !force) {
      ...
      const values = keys.map((k) =>
        Application.getState(`${k}`) as string | undefined
      );
      const [genres, demographic, formats] = values;
      if (
        genres === undefined || demographic === undefined ||
        formats === undefined
      ) {
        await this.updateFilters(true);
        return;
      ...
      this.filter.setDemographicFilter(
        this.parser.parseFilterUpdate(
          await this.api.getFiltersApi("demographic"),
        ),
      );
      ...
      trending_manga: {
        ...
        title: `Trending Manga${
          this.filter.getSectionTimesType()
            ? " of " + this.filter.getYearSettings()
            : ""
        }`,
  ...
  async getDiscoverSectionItems(
    ...
    const fetchSimple = async (id: string) =>
      this.parser.parseSectionSimple(
        page,
        await this.api.getJsonMangaApi(id, page),
      );
    ...
    switch (section.id) {
      ...
      case "follow":
        return this.parser.parseSection(
          section.id,
          await this.api.getJsonMangaTopApi(section.id),
        );
      case "recent":
  ...
  async getSearchResults(
    ...
    sorting.id =
      sorting.id.split(searchQuery.title.length > 1 ? "#title" : "#empty")[0];
    const page = metadata?.page ?? 1;
    ...
    const content = searchQuery.metadata?.contentRating ??
      this.filter.getDefaultContentRatingSettings();
...
export const ComixDMC = new ComixExtension();
