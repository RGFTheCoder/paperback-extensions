...
function classifyConstants(atobBufs: Buffer[]) {
  ...
  const stages: Array<{ sbox: Uint8Array; key: Uint8Array }> = [];
  let pendingSbox: Buffer | null = null;
  for (const buf of atobBufs) {
    if (buf.length === 256) {
      pendingSbox = buf;
    } else if (pendingSbox && buf.length > 0 && buf.length <= 64) {
      stages.push({
        sbox: new Uint8Array(pendingSbox),
...
export function extract() {
  ...
  if (!pt) {
    throw new Error(
      "no TextDecoder output captured — decrypt path did not run",
    );
  }

  const rawStages = classifyConstants(decAtob);
  if (rawStages.length === 0) {
    throw new Error("no (sbox,key) stages found on the atob boundary");
  }
  const N = rawStages.length;
  if (N >= ctLen) {
    throw new Error(
      `probe ciphertext (len ${ctLen}) too short for ${N} rounds — raise ctLen`,
    );
  }
  ...
  const stages = rawStages.map((s, i) => {
    ...
    if (seen.size !== 256) {
      throw new Error(`stage ${i} S-box is not a permutation of 0..255`);
    }
  ...
  for (let i = 0; i < ctLen; i++) {
    if (check[i] !== pt[i]) {
      throw new Error(`decrypt mismatch at byte ${i} after IV solve`);
    }
  }
  ...
  const mine = b64url(
    encryptBytes(new Uint8Array(Buffer.from(PROBE_PATH, "utf8")), stages),
  );
  const signerOk = mine === liveToken;
  if (!signerOk) {
    throw new Error(
      `signer reconstruction failed:\n  live: ${liveToken}\n  mine: ${mine}`,
    );
  }
  ...
  const sample = {
    status: "ok",
    result: { items: [{ id: "9000025", n: "1" }], unicode: "日本語 é → ✓" },
  };
  const e = Buffer.from(
    encryptBytes(
      new Uint8Array(Buffer.from(JSON.stringify(sample), "utf8")),
      stages,
    ),
  ).toString("base64");
  const liveOut = liveDecrypt(resI, e);
  const mineOut = JSON.parse(utf8Decode(decryptBytes(bytes(e), stages)));
  const mineResult = mineOut?.status === "ok" && "result" in mineOut
    ? mineOut.result
    : mineOut;
  const decryptOk = JSON.stringify(liveOut) === JSON.stringify(mineResult);
  if (!decryptOk) {
    throw new Error("decrypt reconstruction did not match live resI");
  }
  ...
  const out = {
    ...
    note:
      "Decrypt applies inverse stages in order 1..N; sign applies forward stages in order N..1. Same (sbox,key,iv) triples both directions.",
    stages: stages.map((s) => ({
...
function main() {
  const out = extract();
  console.log(
    `[extract] bundle ${out.bundleId} — ${out.algorithm}, ${out.rounds} rounds`,
  );
  out.stages.forEach((s, i) =>
    console.log(`  stage ${i + 1}: key ${bytes(s.keyB64).length}B, iv ${s.iv}`)
  );
  console.log(
    `[extract] signer verified: ${out.verified.signer}, decrypt verified: ${out.verified.decrypt}`,
  );
