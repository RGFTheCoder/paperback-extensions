...
const SIGN_PARAM_CASES: Array<[string, Record<string, unknown>]> = [
...
const PAYLOADS = [
  {
    status: "ok",
    result: {
      items: [{ id: "9000025", number: "1", title: "Chapter 1" }],
      meta: { lastPage: 3 },
    },
  },
  {
    ...
    result: {
      pages: [
        "https://x.store/si/abc/001.webp",
        "https://x.store/si/abc/002.webp",
      ],
    },
  },
  {
...
export async function validate() {
  const c: Constants = JSON.parse(readFileSync(CONSTANTS, "utf8"));
  ...
  for (const p of SIGN_PATHS) {
    const live = liveSign(
      reqI,
      p.replace(/^https?:\/\/[^/]+/, "").replace(/^\/api\/v1/, "").split(
        "?",
      )[0],
    );
  ...
  const liveSignParams = (path: string, params: unknown) => {
    const c = {
      url: path,
      method: "get",
  ...
  const buildWire = (obj: Record<string, unknown>) =>
    Object.entries(obj).map(([k, v]) =>
      Array.isArray(v)
        ? v.map((e) => `${k}[]=${encodeURIComponent(e as string)}`).join("&")
        : (v && typeof v === "object")
        ? Object.entries(v).map(([sk, sv]) =>
          `${k}[${sk}]=${encodeURIComponent(sv as string)}`
        ).join("&")
        : `${k}=${encodeURIComponent(v as string)}`
    ).filter(Boolean).join("&");
  ...
  for (const payload of PAYLOADS) {
    ...
    if (
      stableJson(live) === stableJson(mine) &&
      stableJson(mine) === stableJson(expected)
    ) decPass++;
    else {
      decFail++;
      console.log(
        `  DECRYPT FAIL ${payload.status}\n    live: ${
          stableJson(live).slice(0, 80)
  ...
  const ptThrough = fastDecryptComixPayload(
    "/x",
    { status: "ok", result: 1 },
    {},
  );
  if (stableJson(ptThrough) !== stableJson({ status: "ok", result: 1 })) {
    decFail++;
    console.log("  DECRYPT FAIL passthrough(no e)");
  } else decPass++;

  console.log(
    `[validate] signer ${signPass} pass / ${signFail} fail, decrypt ${decPass} pass / ${decFail} fail`,
  );
...
function main() {
  validate().then((r) => Deno.exit(r.ok ? 0 : 1));
}
