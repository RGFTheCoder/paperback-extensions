...
export const getContentRatingMax = async (
  stateManager: SourceStateManager,
): Promise<string> => {
  const val = await stateManager.retrieve("content_rating_max") as
    | string[]
    | null;
  return val?.[0] ?? "suggestive";
};

export const getTrendingLimit = async (
  stateManager: SourceStateManager,
): Promise<string[]> => {
...
export const getUploadersWhitelisted = async (
  ...
  return (await stateManager.retrieve("uploaders_whitelisted") as boolean) ??
    false;
};

export const getStrictNameMatching = async (
  stateManager: SourceStateManager,
): Promise<boolean> => {
  return (await stateManager.retrieve("strict_name_matching") as boolean) ??
    false;
};

export const getUploaders = async (
  stateManager: SourceStateManager,
): Promise<string[]> => {
  return (await stateManager.retrieve("uploaders") as string[]) ?? [];
};

export const getUploaderInput = async (
  stateManager: SourceStateManager,
): Promise<string> => {
  return (await stateManager.retrieve("uploader_input") as string) ?? "";
};

export const getSelectedUploaders = async (
  stateManager: SourceStateManager,
): Promise<string[]> => {
                ...
                App.createDUIButton({
                  ...
                  onTap: async () => {
                    ...
                    if (index !== -1) {
                      ...
                      await stateManager.store("uploaders", uploadersList);
                      const selectedList = await getSelectedUploaders(
                        stateManager,
                      );
                      const newSelected = selectedList.filter((s: string) =>
                        s !== targetUploader
                      );
                      await stateManager.store(
                        "uploaders_selected",
...
export const getTagBlacklist = async (
  stateManager: SourceStateManager,
): Promise<string[]> => {
  return (await stateManager.retrieve("tag_blacklist") as string[]) ?? [];
};

export const getTagFilterEnabled = async (
  stateManager: SourceStateManager,
): Promise<boolean> => {
  return (await stateManager.retrieve("tag_filter_enabled") as boolean) ??
    false;
};

export const getTagWhitelistMode = async (
  stateManager: SourceStateManager,
): Promise<boolean> => {
  return (await stateManager.retrieve("tag_whitelist_mode") as boolean) ??
    false;
};

export const getTagAndMode = async (
  stateManager: SourceStateManager,
): Promise<boolean> => {
...
export const tagFilterSettings = (
  stateManager: SourceStateManager,
  requestManager: RequestManager,
): DUINavigationButton => {
  return keepAlive(App.createDUINavigationButton({
