...
export async function fetchSigned<T>(
  ...
  const apiPath = fullUrl.replace(/^https?:\/\/[^/]+/, "").replace(
    /^\/api\/v1/,
    "",
  );
  const telPath = apiPath.split("?")[0]!;
  const label = /^\/manga\/[^/]+\/chapters/.test(telPath)
    ? "chapters"
    : /^\/chapters\//.test(telPath)
    ? "chapter_images"
    : "signed_fetch";
  ...
  if (json && typeof json === "object" && "e" in json) {
    ...
    return (decrypted && typeof decrypted === "object" &&
        "result" in decrypted && decrypted.status === "ok"
      ? decrypted.result
      : decrypted) as T;
  }
  ...
  if (json.status !== "ok") {
    throw new Error(
      `Comix API ${json.status}: ${json.message ?? "no message"}`,
    );
  }
  emit({
    label,
