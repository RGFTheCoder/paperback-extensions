...
function isImageRequestUrl(url: string): boolean {
  if (!url) return false;
  return /\.(webp|png|jpe?g|avif)(\?|#|$)/i.test(url) ||
...
export class ComixTo extends Source
    ...
    interceptor: {
      ...
      interceptResponse: async (response: Response): Promise<Response> => {
        ...
        try {
          ...
          if (DEBUG) {
            const riff = bytes[0] === 0x52 && bytes[1] === 0x49 &&
              bytes[2] === 0x46 && bytes[3] === 0x46;
            debugLog("img_decrypt", {
              seed: enc.seed,
              len: enc.len,
              algo: enc.algo,
              handled,
  ...
  private async buildFilterParams(): Promise<string> {
    ...
    const [blacklist, whitelistMode, andMode, typeFilterList] = await Promise
      .all([
    ...
    if (typeFilterList.length > 0) {
      const include = whitelistMode
        ? typeFilterList
        : CONTENT_TYPES.map((t) => t.id).filter((t) =>
          !typeFilterList.includes(t)
        );
  ...
  async getMangaDetails(mangaId: string): Promise<SourceManga> {
    const response = await this.fetchTimed(
      "manga_details",
      signUrl(
    ...
    if (json.status !== "ok") {
      throw new Error(
        `Failed to fetch manga details (API ${json.status}: ${
  ...
  override async getViewMoreItems(
    ...
    switch (homepageSectionId) {
      case "trending":
        ...
        url =
          `${API_BASE}/manga/top?type=trending&days=${days}&limit=50&content_rating=${maxRating}${filterParams}`;
        isTopEndpoint = true;
        break;
      case "follows_new":
        url =
          `${API_BASE}/manga/top?type=follows&days=${days}&limit=50&content_rating=${maxRating}${filterParams}`;
        isTopEndpoint = true;
        break;
      case "follows":
        url =
          `${API_BASE}/manga?order[follows_total]=desc&limit=20&page=${page}&includes[]=author${filterParams}`;
        break;
      case "latest":
        url =
          `${API_BASE}/manga?order[chapter_updated_at]=desc&limit=20&page=${page}&includes[]=author${filterParams}`;
        break;
      case "new":
        url =
          `${API_BASE}/manga?order[created_at]=desc&limit=20&page=${page}&includes[]=author${filterParams}`;
        break;
      default:
    ...
    if (cached) {
      ...
      const fetchTags = async (type: string) => {
        try {
          ...
          const res = await this.fetchTimed(
            "search_tags",
            signUrl(`${API_BASE}/tags/search?type=${type}&limit=50`),
          );
          if (res.status < 200 || res.status >= 300) return [];
          const json = JSON.parse(res.data ?? "{}") as APIResponse<
            APIGenreResult
          >;
  ...
  async getSearchResults(
    ...
    const orderTag = (query.includedTags ?? []).find((t) =>
    ...
    const stripPrefix = (id: string) =>
      id.replace(/^(genre-|tag-|format-|demographic-)/, "");
    const isTag = (id: string) => TAG_PREFIXES.some((p) => id.startsWith(p));

    const includedTagIds = allTags.filter((t) => isTag(t.id)).map((t) =>
      stripPrefix(t.id)
    );
    const excludedTagIds = excludedTags.filter((t) => isTag(t.id)).map((t) =>
      stripPrefix(t.id)
    );
    const typeIds = allTags.filter((t) => t.id.startsWith("type-")).map((t) =>
      t.id.replace("type-", "")
    );
    const statusIds = allTags.filter((t) => t.id.startsWith("status-")).map(
      (t) => t.id.replace("status-", ""),
    );
  ...
  checkResponseError(response: Response): void {
    ...
    if (response.status < 200 || response.status >= 300) {
      throw new Error(
        `HTTP ${response.status}: Unexpected response from server [${ctx}]`,
      );
    }
